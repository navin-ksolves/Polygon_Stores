...
class ShopifyInstanceEpt(models.Model):
    ...
    shopify_tag_ids = fields.Many2many('shopify.tags', 'shopify_ept_shopify_tags', string='Shopify Tags')
      
    shopify_activity_type_id = fields.Many2one('mail.activity.type',
                                               string="Activity Type")
    shopify_contact_id = fields.Many2one('res.partner',
                                               string="Contact")
    ...
    shopify_location_id = fields.Many2one('shopify.location.ept', string='Default Shopify Location')
    ...
    def refresh_webhooks(self):
        ...
        _logger.info("Webhook: Current webhook present in shopify is %s", webhook_ids)
        webhook_obj = self.env['shopify.webhook.ept'].search(
            [('instance_id', '=', self.id), ('webhook_id', 'not in', webhook_ids)])
        _logger.info("Webhook: Webhook not present in odoo is %s", webhook_obj)

        if webhook_obj:
            for webhooks in webhook_obj:
                _logger.info("Webhook: deleting the %s shopify.webhook.ept record", webhooks.id)
                self._cr.execute("DELETE FROM shopify_webhook_ept WHERE id = %s", [webhooks.id], log_exceptions=False)
        _logger.info("Webhook: refresh process done")
        return True
