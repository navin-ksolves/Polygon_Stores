...
class SaleOrder(models.Model):
    ...
    shopify_shipping_web_id = fields.Char("Shopify Shipping Method", copy=False)
    ...
    def set_shopify_location_and_warehouse(self, order_response, instance, pos_order):
        ...
        #print("Glihhhhhhhhhhhhhhhhhhhhh",order_response)
    ...
    def import_shopify_orders(self, order_data_lines, instance):
        ...
        for order_data_line in order_data_lines:
            ...
            ship_name = order_response.get("shipping_lines")
            tz = sale_order.warehouse_id.tz
           
            user_id = self.env['res.users'].search([('id','=',2)])
            local = pytz.timezone(user_id.tz)
            _logger.info("TIME ZONE===>>>>>>>>>>>>>>>>{}".format(user_id.tz))
            for ship in ship_name:
                if ship:
                    ship_id = ship.get("title")
                    commitment_date = datetime.now()
                    if ship_id == "Instant 1hr":
                        delivery_carrier_id = self.env['delivery.carrier'].search([('shipping_method','=','express')], limit=1)
                        # shipping_method = "express"
                        commitment_date = sale_order.date_order
                    elif ship_id == "Next Day":
                        delivery_carrier_id = self.env['delivery.carrier'].search([('shipping_method','=','next_day')], limit=1)
                        # shipping_method = "next_day"
                        commitment_date = sale_order.date_order + timedelta(days = 1)
                        commitment_date = commitment_date.replace(hour=9, minute=00)
                       
                    elif ship_id == "Same Day":
                        # shipping_method = "same_day"
                        delivery_carrier_id = self.env['delivery.carrier'].search([('shipping_method','=','same_day')], limit=1)                      
                        if self.warehouse_id.shift_slot_start > float(sale_order.date_order.hour) and self.warehouse_id.shift_slot_end < float(sale_order.date_order.hour):
                            commitment_date = sale_order.date_order
                            commitment_date = commitment_date.replace(hour=00, minute=00)
                            commitment_date = commitment_date + timedelta(hours=self.warehouse_id.shift_slot_end)
                        elif self.warehouse_id.shift_slot_start < float(sale_order.date_order.hour):
                            commitment_date = sale_order.date_order
                            commitment_date = commitment_date.replace(hour=00, minute=00)
                            commitment_date = commitment_date + timedelta(hours=self.warehouse_id.shift_slot_start)
                        elif self.warehouse_id.shift_slot_end > float(sale_order.date_order.hour):
                            commitment_date = sale_order.date_order + timedelta(days=1)
                            commitment_date = commitment_date.replace(hour=00, minute=00)
                            commitment_date = commitment_date + timedelta(hours=self.warehouse_id.shift_slot_start)
                    
                    commitment_date = fields.Datetime.to_string(local.localize(fields.Datetime.from_string(commitment_date), is_dst=None).astimezone(pytz.utc))
                    _logger.info("\n\n\nCOMMITMENT DATE===>>>>>>>>>>>>>>>>{}".format(commitment_date))
                    if ship_id:
                        location_vals.update({
                            "shopify_shipping_web_id" :ship_id,
            ...
            location_id = self.env['shopify.location.ept'].search([("import_stock_warehouse_id","=",sale_order.warehouse_id.id),("instance_id","=",instance.id)])
            if location_id:
                sale_order.write({"shopify_location_id":location_id.id})
        ...
        if sale_order:
            if sale_order.shopify_shipping_web_id in ['Same Day', 'Next Day']:

                for rec in sale_order.picking_ids:
                    rec.write({
                        'date_deadline': rec.scheduled_date + timedelta(hours=1)
                    })
        return order_ids
    ...
    def prepare_order_vals_from_order_response(self, order_response, instance, workflow, payment_gateway):
        ...
        financial_status_id = self.env['sale.auto.workflow.configuration.ept'].search([
        ...
        order_vals = {
            ...
            "tag_ids": tag_ids,
            "payment_term_id": financial_status_id.payment_term_id.id
        }
        return order_vals
    ...
    def update_order_status_in_shopify(self, instance, picking_ids=[]):
        ...
        for picking_id in picking_ids:
            if picking_id.state != 'done':
                raise UserError('Some of the picking of this order is not fulfilled.')
        for picking in picking_ids:
            ...
            _logger.info("ORDER RESPONSE===>>>>>>>>>>>>>>>>>>>{}".format(order_response))
            if order_response:
                if order_response.get('fulfillment_status'):
                    sale_order.write({
            ...
            shopify_location_id = sale_order.shopify_location_id
    ...
    def fulfilled_picking_for_shopify(self, pickings):
        ...
        for picking in pickings.filtered(lambda x: x.state not in ['cancel', 'done']):
            if self.delivery_carrier_id.shipping_method == "next_day":
                date_deadline = picking.scheduled_date + timedelta(hours=1)
